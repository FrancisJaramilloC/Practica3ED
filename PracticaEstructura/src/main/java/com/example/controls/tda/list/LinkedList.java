...
public class LinkedList<E> {
    ...
    public static Integer MAYOR = 1;
    public static Integer MENOR = 0;
    ...
    //Obtener un nodo especifico(privado)
    ...
    //Obtener un nodo especifico(publico)
    ...
    //Agregar un nodo a una posicion especifica
    ...
    // Actualizar un nodo especifico
    public void update(E data, Integer post) throws Exception{
        if (isEmpty()) {
            throw new ListEmptyException("Error, lista vacia");
        }else if (post < 0 || post >= size){
            throw new IndexOutOfBoundsException("Error, lista fuera de rango");
        }else if (post == 0){
            header.setInfo(data);
        }else if (post == (size-1)){
            last.setInfo(data);
    ...
    //Implementacion metodo MergeSort 
    ...
    private void merge(E[] arr, int left, int mid, int right, Comparator<E> comparator) {
        ...
        // Data de los arreglos
        for (int i = 0; i < n1; ++i)
            leftArray[i] = arr[left + i];
        for (int j = 0; j < n2; ++j)
            rightArray[j] = arr[mid + 1 + j];

        // Merge de los arreglos temporales
        int i = 0, j = 0, k = left;
        while (i < n1 && j < n2) {
    ...
    // Implementacion del Metodo ShellSort
